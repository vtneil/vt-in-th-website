...
function sanity_check_inputs(mb, mp, mp_set, tar, tba, tar_set, tba_set) {
    ...
    if (tba_set && tba < 10000) {
        ...
        return 1;
    } else if (tba_set && tba > 55000) {
        set_error('tba',
                "Target burst altitude is too high! (greater than 55km)");
        return 1;
    }
...
function calc_update() {
    ...
    let mp = get_value('mp');
    const tar = get_value('tar');
    const tba = get_value('tba');
    let mp_set = 0;
    let tar_set = 0;
    let tba_set = 0;
    ...
    const rho_g = find_rho_g();
    const rho_a = get_value('rho_a');
    const adm = get_value('adm');
    const ga = get_value('ga');
    const bd = find_bd(mb);
    const cd = find_cd(mb);
    ...
    // Do some maths
    mb = parseFloat(mb.substring(1)) / 1000.0;
    mp = mp / 1000.0;

    let ascent_rate = 0;
    let burst_altitude = 0;
    let time_to_burst = 0;
    let neck_lift = 0;
    let launch_radius = 0;
    let launch_volume = 0;

    const burst_volume = (4.0 / 3.0) * Math.PI * Math.pow(bd / 2.0, 3);

    if (tba_set) {
        launch_volume = burst_volume * Math.exp((-tba) / adm);
        launch_radius = Math.pow((3 * launch_volume) / (4 * Math.PI), (1 / 3));
    } else if (tar_set) {
        const a = ga * (rho_a - rho_g) * (4.0 / 3.0) * Math.PI;
        const b = -0.5 * Math.pow(tar, 2) * cd * rho_a * Math.PI;
        const c = 0;
        const d = -(mp + mb) * ga;

        const f = (((3 * c) / a) - (Math.pow(b, 2) / Math.pow(a, 2)) / 3.0);
        const g = (((2 * Math.pow(b, 3)) / Math.pow(a, 3)) -
                ((9 * b * c) / (Math.pow(a, 2))) + ((27 * d) / a)) / 27.0;
        const h = (Math.pow(g, 2) / 4.0) + (Math.pow(f, 3) / 27.0);

        if (h <= 0)
            throw "expect exactly one real root";

        const R = (-0.5 * g) + Math.sqrt(h);
        const S = Math.pow(R, 1.0 / 3.0);
        const T = (-0.5 * g) - Math.sqrt(h);
        const U = Math.pow(T, 1.0 / 3.0);
        launch_radius = (S + U) - (b / (3 * a));
    }

    const launch_area = Math.PI * Math.pow(launch_radius, 2);
    launch_volume = (4.0 / 3.0) * Math.PI * Math.pow(launch_radius, 3);
    const density_difference = rho_a - rho_g;
    const gross_lift = launch_volume * density_difference;
    neck_lift = (gross_lift - mb) * 1000;
    const total_mass = mp + mb;
    const free_lift = (gross_lift - total_mass) * ga;
    ascent_rate = Math.sqrt(free_lift / (0.5 * cd * launch_area * rho_a));
    const volume_ratio = launch_volume / burst_volume;
    ...
    const launch_litres = (launch_volume * 1000).toFixed();
    const launch_cf = (launch_volume * 35.31).toFixed(1);
...
$(document).ready(function () {
    ...
    const ids = ['mb', 'mp', 'tar', 'tba', 'gas', 'rho_g', 'rho_a', 'adm', 'bd', 'cd', 'bd_c', 'cd_c', 'ga'];
